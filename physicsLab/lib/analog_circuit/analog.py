...
# 全部节点间导线，目前用于位置排布和确定空探针的连接关系
_gicw: Dict[_Experiment, Set[FrozenSet["Vertex"]]] = {}

# 记录某类节点的数目，在给节点自动取名时使用，方便调试
_names: Dict[_Experiment, Dict[str, int]] = {}
...
class Node:
    ...
    def shift(
        ...
        for i in self.elements:
            ...
            if i.is_elementXYZ:
                ...
                xp, yp, zp = (
                    _tools.round_data(xp),
                    _tools.round_data(yp),
                    _tools.round_data(zp),
                )
    ...
    def locate(
        ...
        self.shift(
            x - self._pos.x, y - self._pos.y, z - self._pos.z, elementXYZ=elementXYZ
        )
        return self
        ...
        if len(nodes) == 0:
            self.locate(
                ...
                (
                    12
                    if same_level_y == []
                    else min(same_level_y) - self.height / 2 - space
                )
                - self.height / 2,
                z,
            )
            ...
            self.locate(
                nodes[0].pos.x + nodes[0].width / 2 + self.width / 2 + space,
                nodes[0].pos.y,
                z,
            )
        elif len(nodes) == 2:
            if (
                abs(nodes[0].pos.x - nodes[1].pos.x)
                + nodes[0].width / 2
                + nodes[1].width / 2
                ...
                self.locate(
                    max(
                        nodes[0].pos.x + nodes[0].width / 2,
                        nodes[1].pos.x + nodes[1].width / 2,
                    )
                    + self.width / 2
                    + space,
                    nodes[0].pos.y / 2 + nodes[1].pos.y / 2,
                    z,
                )
            elif (
                abs(nodes[0].pos.y - nodes[1].pos.y)
                + nodes[0].height / 2
                + nodes[1].height / 2
                ...
                self.locate(
                    right.pos.x + right.width / 2 + self.width / 2 + space,
                    right.pos.y,
                    z,
                )
            else:
                ...
                self.locate(
                    down.pos.x + down.width / 2 + self.width / 2 + space, down.pos.y, z
                )
            for i in _gn[expe]:
                if i.pos == self.pos:
                    i.shift(0, 0.5 + i.height / 2, 0)
                    self.shift(0, -0.5 - i.height / 2, 0)
        else:
    ...
    def __repr__(self) -> str:
        return "Node(name={}, input={}, output={}).extend({}, {})".format(
            self.name, self.input, self.output, self.elements, self.wires
        )
    ...
    def __mul__(
        self, other: Union["Node", num_type]
    ) -> Union["ComplexNode", "LinearNode"]:
        if isinstance(other, (int, float)):
...
def name_gen(name: str) -> str:
    ...
    if _names.get(expe) is None:
        _names[expe] = {name: 1}
    elif _names.get(expe).get(name) is None:
        _names[expe][name] = 1
    else:
        _names[expe][name] += 1
...
def node_wrapper(name: str) -> Callable:
    ...
    def decorator_node_wrapper(func: FunctionType) -> Callable:
        def wrapper(*args, **kwargs) -> ComplexNode:
            expe = get_current_experiment()
            ...
            if not isinstance(res, Node):
                raise TypeError(
                    "The return value of the function must be a Node object"
                )
            ...
            for i in list(args) + list(kwargs.values()):
                if isinstance(i, Node):
                    ...
                    for (
                        ...
                        all_vertex_wires
                    ):  # 这里必须用vertex_wires，因为可能有试探用的空接线柱
                        if i.output in k:
                            inputs |= k - {i.output}
                            # wires.remove(k)
            if input_nodes == []:
...
class PinNode(Node):
    ...
    def __init__(self, pin: Pin, gnd: Ground_Component) -> None:
        ...
        super().__init__(
            pos.x, pos.y, pos.z, "pin", gnd, elementXYZ=pin.element_self.is_elementXYZ
        )
        self.input = [pin]
        self.output = pin
...
class VoidNode(Node):
    ...
    def __init__(self, x, y, z, gnd, /, *, elementXYZ=None):
        super().__init__(x, y, z, name_gen("void"), gnd, elementXYZ=elementXYZ)
...
class SubNode(Node):
    ...
    def __init__(self, x, y, z, gnd: Ground_Component) -> None:
        super().__init__(x, y, z, name_gen("sub"), gnd)
        x, y, z = self._pos

        r11 = Resistor(x - 1, y + 0.5, z, elementXYZ=True, resistance=99)
        r12 = Resistor(x, y + 0.5, z, elementXYZ=True, resistance=1)
        r21 = Resistor(x - 1, y - 0.5, z, elementXYZ=True, resistance=99)
        r22 = Resistor(x, y - 0.5, z, elementXYZ=True, resistance=1)
...
class LinearNode(Node):
    ...
    def __init__(
        self, x, y, z, gnd: Ground_Component, k: num_type, b: num_type = 0
    ) -> None:
        super().__init__(x, y, z, name_gen("linear"), gnd)
        x, y, z = self._pos
        scale = b == 0
        ...
        if k == 0:
            ...
            oa1 = Operational_Amplifier(
                x + 0.5 if scale else x + 1, y, z, elementXYZ=True, gain=100
            )
            r1 = Resistor(
                x - 0.5 if scale else x,
                y - 0.5 if scale else y + 0.5,
                z,
                elementXYZ=True,
                resistance=1,
            )
            r2 = Resistor(
                x - 0.5 if scale else x - 1,
                y + 0.5,
                z,
                elementXYZ=True,
                resistance=100 / k - 1,
            ...
            oa1 = Operational_Amplifier(
                x + 0.5 if scale else x + 1, y, z, elementXYZ=True, gain=k
            )
...
class PrimitiveLogTransistorNode(Node):
    ...
    def __init__(self, x, y, z, gnd: Ground_Component) -> None:
        super().__init__(x, y, z, name_gen("primitive_log_transistor"), gnd)
        x, y, z = self._pos

        oa1 = Operational_Amplifier(
            x + 0.5, y - 0.5, z, elementXYZ=True, max_voltage=15, min_voltage=-15
        )
        r1 = Resistor(x - 0.5, y - 0.5, z, elementXYZ=True)
        t1 = Transistor(x + 0.5, y + 1, z, elementXYZ=True, is_PNP=False)
...
class IntNode(Node):
    def __init__(self, x, y, z, gnd: Ground_Component) -> None:
        """积分节点 (y=∫xdt)"""
        """ 来源：@MapMaths """
        super().__init__(x, y, z, name_gen("int"), gnd)
        x, y, z = self._pos
        ...
        r1 = Resistor(x - 0.5, y - 0.5, z, elementXYZ=True)
        c1 = Basic_Capacitor(
            x + 0.5, y + 1, z, elementXYZ=True, capacitance=0.1, is_ideal=True
        )
...
class DifNode(Node):
    def __init__(self, x, y, z, gnd: Ground_Component) -> None:
        """微分节点 (y=dx/dt)"""
        """ 来源：@MapMaths """
        super().__init__(x, y, z, name_gen("dif"), gnd)
        x, y, z = self._pos

        oa1 = Operational_Amplifier(
            x + 0.5, y - 0.5, z, elementXYZ=True, max_voltage=15, min_voltage=-15
        )
        r1 = Resistor(x + 0.5, y + 1, z, elementXYZ=True)
        c1 = Basic_Capacitor(
            x - 0.5, y - 0.5, z, elementXYZ=True, capacitance=0.1, is_ideal=True
        )
...
def quadrant(
    func: FunctionType, vertex_id1: int = 0, vertex_id2: int = 1
) -> FunctionType:
    ...
    def res_func(n1: Node, n2: Node):
        ...
        for i in _gicw[expe].copy():
            if (
                list(i)[0] is v1.output
                or list(i)[1] is v1.output
                or list(i)[0] is v2.output
                or list(i)[1] is v2.output
            ):
                _gicw[expe].remove(i)
        return node
...
class MOSMultiplierNode(Node):
    ...
    def __init__(self, x, y, z, gnd: Ground_Component):
        super().__init__(x, y, z, name_gen("mos_multiplier"), gnd)
        x, y, z = self._pos

        r1 = Resistor(x - 1, y + 0.5, z, elementXYZ=True, resistance=99999999)
        r2 = Resistor(x, y + 0.5, z, elementXYZ=True, resistance=1)
        ...
        oa2 = Operational_Amplifier(x + 1, y + 0.5, z, elementXYZ=True, gain=100)
...
def log(n: Union[Node, num_type], n_base: Union[Node, num_type]) -> ComplexNode:
    """对数"""
    if not isinstance(n, (Node, int, float)) or not isinstance(
        n_base, (Node, int, float)
    ):
    ...
    if not x_is_num and not base_is_num:
        return (pri_log(n) + 0.69077552617129989) / (
            pri_log(n_base) + 0.69077552617129989
        )
