...
class Super_AndGate:
    ...
    def __init__(
        ...
        *,
        ...
        _muls: List[elements.Multiplier] = [
            elements.Multiplier(x, y, z, elementXYZ=True) for _ in range(div_num)
        ]
        self._inputs = []
        for a_mul in _muls:
        ...
        if mod_num == 0:
            ...
            crt_wires(
                UnitPin(None, *[a_mul.o_up for a_mul in _muls]),
                sub_super_and_gate.inputs,
            )
        elif mod_num == 1:
            sub_super_and_gate = Super_AndGate(
                x, y, z, bitnum=div_num + 1, elementXYZ=True
            )
            self._inputs.append(sub_super_and_gate._inputs[-1])
            crt_wires(
                UnitPin(None, *[a_mul.o_up for a_mul in _muls]),
                sub_super_and_gate.inputs[:-1],
            )
        elif mod_num == 2:
            sub_super_and_gate = Super_AndGate(
                x, y, z, bitnum=div_num + 1, elementXYZ=True
            ...
            crt_wires(
                UnitPin(None, *[a_mul.o_up for a_mul in _muls], _and_gate.o),
                sub_super_and_gate.inputs,
            )
        elif mod_num == 3:
            sub_super_and_gate = Super_AndGate(
            ...
            crt_wires(
                UnitPin(None, *[a_mul.o_up for a_mul in _muls], _mul.o_up),
                sub_super_and_gate.inputs,
            )
        else:
            errors.unreachable()
    ...
    def __init__(
        ...
        *,
        ...
        if bitnum % 2 == 0:
            num_copy = bitnum
            while num_copy != 0:
                tmp = elements.Or_Gate(
                    x, y + (bitnum - num_copy) / 2, z, elementXYZ=True
                )
                self._inputs += [tmp.i_low, tmp.i_up]
                self._outputs.append(tmp.o)
                num_copy -= 2
            next_orgates = Super_OrGate(
                x, y, z, bitnum=len(self._outputs), elementXYZ=True
            )
            ...
            while num_copy != 1:
                ...
                num_copy -= 2
            next_orgates = Super_OrGate(
                x, y, z, bitnum=len(self._outputs) + 1, elementXYZ=True
            )
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            or not isinstance(y, (int, float))
            or not isinstance(z, (int, float))
            or elementXYZ is not None
            and not isinstance(elementXYZ, bool)
            or not isinstance(bitnum, int)
            or bitnum <= 1
        ):
            raise TypeError
        ...
        if num == 2:
            ...
            for i, a_bit in enumerate(bitlist):
                if a_bit:
                    self._o = [
                        self._output.o_low,
                        self._output.o_lowmid,
                        self._output.o_upmid,
                        self._output.o_up,
                    ][i]
            ...
            if len(output_pins) >= 2:
                sa = Super_AndGate(
                    x + 1, y, z, bitnum=len(output_pins), elementXYZ=True
                )
                self._o = sa.output
